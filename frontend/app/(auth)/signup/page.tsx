...
export default function SignUpPage() {
    ...
    >(async (prevState: ActionResponse, formData: FormData) => {
        try {
            ...
            if (result.success) {
                // Store token in cookie if it's returned
                if (result.token) {
                    // Set the secure cookie
                    document.cookie = `auth_token=${result.token}; path=/; max-age=${60 * 60 * 24 * 7}; samesite=lax${window.location.protocol === 'https:' ? '; secure' : ''
                        }`;
                }
